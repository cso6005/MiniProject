...
public class OttService {
	...
	public static boolean selectContents() throws Exception {
		try {
			...
			for (Contents c : contentsList) {
				if (c.getContentsName().equals(inputTitle)) {
					System.out.println(c.getContentsName());
					EndView.contentsSView(c);
					logger.debug(c.getContentsName() + "검색 하셨습니다.");
					return false;
				}
			}
	...
	public static void returnContentsList() throws Exception {
		...
		try {
			result = ContentsDAO.returnContentsList(connUser.getGenreName());
			if (result != null) {
				System.out.println("⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯");
	...
	public static boolean recommendContents() throws Exception {
		try {
			...
			for (Users u : usersList) {
				if (u.getGenreName() != null && u.getName().equals(connUser.getName())) {
					for (Contents c : contentsList) {
						if (c.getGenre_name().equals(u.getGenreName())) {
